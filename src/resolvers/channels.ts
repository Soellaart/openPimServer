...
export default {
  Query: {
    ...
    getChannels: async (parent: any, args: any, context: Context) => {
      ...
      cloned.forEach((channel: Channel) => {
        if (channel.type === 2 && channel.config.wbToken) {
          // WB
          channel.config.wbToken = '*****';
        } else if (channel.type === 3 && channel.config.ozonApiKey) {
          // Ozon
      ...
      if (!context.canViewChannel(chan.identifier)) {
        throw new Error(
          'User ' +
            ...
            ' does not has permissions to view channel, tenant: ' +
    ...
    getExecutionById: async (parent: any, { id }: any, context: Context) => {
      ...
      if (!context.canViewChannel(channel.identifier)) {
        throw new Error(
          `User ${context.getCurrentUser()?.id} does not have permissions to view channel with identifier ${channel.identifier}, tenant: ${
            context.getCurrentUser()!.tenantId
          }`,
        );
      }
  ...
  Mutation: {
    triggerChannel: async (parent: any, { id, language, data }: any, context: Context) => {
      ...
      if (!context.canEditChannel(chan.identifier) || chan.tenantId !== context.getCurrentUser()?.tenantId) {
        throw new Error(
          'User ' +
            context.getCurrentUser()?.id +
            ' does not has permissions to triger channel, tenant: ' +
            context.getCurrentUser()!.tenantId,
        );
      }
      const channelMng = ChannelsManagerFactory.getInstance().getChannelsManager(context.getCurrentUser()!.tenantId);
    ...
    createChannel: async (
      ...
      if (!context.canEditConfig(ConfigAccess.CHANNELS))
        throw new Error(
          'User ' +
            context.getCurrentUser()?.id +
            ' does not has permissions to create channel, tenant: ' +
            context.getCurrentUser()!.tenantId,
        );

      if (!/^[A-Za-z0-9_]*$/.test(identifier))
        throw new Error(
          'Identifier must not has spaces and must be in English only: ' + identifier + ', tenant: ' + context.getCurrentUser()!.tenantId,
        );
    ...
    updateChannel: async (
      ...
      if (!context.canEditConfig(ConfigAccess.CHANNELS))
        throw new Error(
          'User ' +
            context.getCurrentUser()?.id +
            ' does not has permissions to update channel, tenant: ' +
            context.getCurrentUser()!.tenantId,
        );
      ...
      if (config) {
        if (type === 2 && config.wbToken === '*****') {
          // WB
          config.wbToken = chan.config.wbToken;
        } else if (type === 3 && config.ozonApiKey === '*****') {
          // Ozon
    ...
    removeChannel: async (parent: any, { id }: any, context: Context) => {
      ...
      if (!context.canEditConfig(ConfigAccess.CHANNELS))
        throw new Error(
          'User ' +
            context.getCurrentUser()?.id +
            ' does not has permissions to remove channel, tenant: ' +
            context.getCurrentUser()!.tenantId,
        );
    ...
    bulkUpdateChannels: async (params: any, { identifiers, status, message, where }: any, context: Context) => {
      ...
      const chanRestrictSQL = identifiers
        .map((ident: any) => {
          // filter by type and path valid for channel
          const chan = mng.getChannels().find((chan) => chan.identifier === ident);
          if (chan) {
      ...
      if (status !== 0) {
        const param = identifiers
          .map((identifier: any) => {
            return `{ "${identifier}": { "status": ${status}, "submittedAt": ${Date.now()}, "submittedBy": "${
              context.getCurrentUser()!.login
