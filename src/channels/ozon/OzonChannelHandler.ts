/**
 * OzonChannelHandler class processes items for syncing with Ozon.
 * It handles:
 *  - Checking channel configuration (Ozon Client ID, API key, ID attribute).
 *  - Uploading products, updating statuses, categories, prices, images, and videos.
 *  - Synchronizing items in bulk or individually, with error and rating handling.
 *  - Caching data and providing category/attribute information.
 */
...
export class OzonChannelHandler extends ChannelHandler {
    ...
    public async processChannel(channel: Channel, language: string, data: any): Promise<void> {
        ...
        if (!channel.config.ozonClientId) {
            await this.finishExecution(channel, chanExec, 3, 'Client ID not provided in channel configuration');
            return;
        }
        if (!channel.config.ozonApiKey) {
            await this.finishExecution(channel, chanExec, 3, 'API key not provided in channel configuration');
            return;
        }
        if (!channel.config.ozonIdAttr) {
            await this.finishExecution(channel, chanExec, 3, 'Attribute to store Ozon ID not provided');
            return;
        }
        ...
        try {
            if (!data) {
                ...
                context.log += 'Ozon export started\n';
    ...
    async syncJob(channel: Channel, context: JobContext, data: any) {
        context.log += 'Ozon synchronization started\n';
    ...
    processProductStatus(item: Item, result: any, channel: Channel, context: JobContext) {
        ...
        context.log += '   product status: ' + JSON.stringify(status);
        ...
        if (status.is_created && !status.is_failed && status.moderate_status !== 'declined') {
            ...
            item.channels[channel.identifier].message = 'Moderation: ' + JSON.stringify(status);
            item.channels[channel.identifier].syncedAt = new Date().getTime();
            item.changed('channels', true);
        }
    ...
    async syncItem(channel: Channel, item: Item, context: JobContext, singleSync: boolean) {
        ...
        if (item.values[channel.config.ozonIdAttr] && item.channels[channel.identifier]) {
            ...
            if (chanData.status === 3 && !chanData.ozonError) {
                context.log += 'Product status - error, synchronization will not be performed \n';
                return;
            }
            ...
            if (tst.startsWith('task_id=')) {
                ...
                if (res2.status !== 200) {
                    ...
                    const msg = 'Ozon request error: ' + res2.statusText + ' ' + text;
                    context.log += msg;
                    ...
                    if (json2.result.items.length === 0 || json2.result.items[0].product_id == 0) {
                        context.log += '  product with identifier ' + item.identifier + ' has not yet received product_id \n';
                        return;
                    } else {
            ...
            if (res.status !== 200) {
                const msg = 'Ozon request error: ' + res.statusText;
                context.log += msg;
                return;
            } else {
                ...
                if (data.items.length == 0) {
                    context.log += '  ERROR: Product not found on Ozon with product id ' + item.values[channel.config.ozonIdAttr] + ' \n';
                    return;
                }
                ...
                if (result.sku) {
                    ...
                    context.log += '  product with identifier ' + item.identifier + ' has not yet received sku (rating retrieval unavailable) \n';
                }
                this.processProductStatus(item, result, channel, context);
            }

            await this.saveItemIfChanged(channel, item);
            context.log += '  product with identifier ' + item.identifier + ' synchronized \n';
            if (channel.config.ozonAttrContentRating && channel.config.ozonGetContentRating && sku) {
                ...
                if (resRating.status !== 200) {
                    const msg = 'Ozon request error: ' + resRating.statusText;
                    context.log += msg + '\n';
                    logger.error(msg);
                    return;
                }
                ...
                context.log += '  rating for product with identifier ' + item.identifier + ' synchronized\n';
            }
        } else {
    ...
    async syncItems(channel: Channel, items: Item[], context: JobContext) {
        context.log += 'Processing products with identifiers: [' + items.map(item => item.identifier).join(', ') + ']\n';
        ...
        if (productIds.length === 0) {
            context.log += 'No products to synchronize\n';
            return;
        }
        ...
        for (let i = 0; i < productIds.length; i += chunkSize) {
            ...
            for (const item of filteredItems) {
                ...
                if (!result) {
                    context.log += 'Product with identifier ' + item.identifier + ' not found in Ozon response\n';
                    continue;
                }
                ...
                if (item.channels[channel.identifier]?.status === 3 && !item.channels[channel.identifier]?.ozonError) {
                    context.log += 'Product status ' + item.identifier + ' error, synchronization will not be performed \n';
                    continue;
                }
                this.processProductStatus(item, result, channel, context);
                await this.saveItemIfChanged(channel, item);
        ...
        if (channel.config.ozonAttrContentRating && channel.config.ozonGetContentRating) {
            for (let j = 0; j < skus.length; j += chunkSizeSku) {
                ...
                for (const item of filteredItems) {
                    ...
                    if (!result) {
                        context.log += 'Product with identifier ' + item.identifier + ' not found in Ozon response (Rating)\n';
                        continue;
                    }
        ...
        context.log += 'Product synchronization finished\n';
    }
    ...
    async processItem(channel: Channel, item: Item, language: string, context: JobContext) {
        ...
        for (const categoryId in channel.mappings) {
            ...
            if (
              categoryConfig.valid &&
              categoryConfig.valid.length > 0 &&
              ((categoryConfig.visible && categoryConfig.visible.length > 0) ||
                categoryConfig.categoryExpr ||
                (categoryConfig.categoryAttr && categoryConfig.categoryAttrValue))
            ) {
                const pathArr = item.path.split('.');
                const tstType =
                  categoryConfig.valid.includes(item.typeId) ||
                  categoryConfig.valid.includes('' + item.typeId);
                if (tstType) {
                    let tst = null;
                    if (categoryConfig.visible && categoryConfig.visible.length > 0) {
                        if (categoryConfig.visibleRelation) {
                            let sources = await Item.findAll({
                                where: {
                                    ...
                                    '$sourceRelation.relationId$': categoryConfig.visibleRelation,
                                    '$sourceRelation.targetId$': item.id
                                },
                        ...
                        tst =
                          item.values[categoryConfig.categoryAttr] &&
                          item.values[categoryConfig.categoryAttr] == categoryConfig.categoryAttrValue;
                    }
                    if (tst) {
                        try {
                            const changedValues = await this.processItemInCategory(
                              channel,
                              item,
                              categoryConfig,
                              language,
                            ...
                            logger.error(
                              'Failed to process item with id: ' + item.id + ' for tenant: ' + item.tenantId,
                              err
                            );

                            const data = item.channels[channel.identifier];
                            data.status = 3;
                            data.message = 'Product processing error: ' + err;
                ...
                // This record does not match the category config
            }
        }
        ...
        context.log +=
          'Record with identifier:' + item.identifier + ' does not match any category in this channel.\n';
        await this.saveItemIfChanged(channel, item);
    }

    async saveItemIfChanged(channel: Channel, item: Item, changedValues: any = {}) {
        const reloadedItem = await Item.findByPk(item.id); // refresh item from DB (other channels may have changed it)
        let changed = false;
        let valuesChanged = false;
        const data = item.channels[channel.identifier];
        const newChannels: any = {};
        newChannels[channel.identifier] = JSON.parse(
          JSON.stringify(reloadedItem!.channels[channel.identifier])
        );
        ...
        if (
          reloadedItem!.values[channel.config.ozonIdAttr] !==
          item.values[channel.config.ozonIdAttr]
        ) {
            changed = true;
            valuesChanged = true;
        ...
        if (
          reloadedItem!.values[channel.config.ozonFBSIdAttr] !==
          item.values[channel.config.ozonFBSIdAttr]
        ) {
            changed = true;
            valuesChanged = true;
        ...
        if (
          reloadedItem!.values[channel.config.ozonFBOIdAttr] !==
          item.values[channel.config.ozonFBOIdAttr]
        ) {
            changed = true;
            valuesChanged = true;
        ...
        if (
          reloadedItem!.values[channel.config.ozonAttrContentRating] !==
          item.values[channel.config.ozonAttrContentRating]
        ) {
            changed = true;
            valuesChanged = true;
            changedValues[channel.config.ozonAttrContentRating] =
              item.values[channel.config.ozonAttrContentRating];
        }
        if (
          reloadedItem!.values[channel.config.ozonCategoryAttr] !==
          item.values[channel.config.ozonCategoryAttr]
        ) {
            changed = true;
            valuesChanged = true;
        ...
        if (changed) {
            ...
            try {
                await processItemActions(
                  ctx,
                  EventType.BeforeUpdate,
                  reloadedItem!,
                  reloadedItem!.parentIdentifier,
                  reloadedItem!.name,
                  changedValues,
                  newChannels,
                  false,
    ...
    async processItemInCategory(channel: Channel, item: Item, categoryConfig: any, language: string, context: JobContext) {
        context.log +=
          'Category "' +
          categoryConfig.name +
          '" found for record with identifier: ' +
          item.identifier +
          '\n';
        ...
        if (!productCode) {
            const msg =
              'No configuration or data provided for "Product Article" for category: ' +
              categoryConfig.name;
            context.log += msg;
            this.reportError(channel, item, msg);
            return;
        ...
        if (!vat) {
            const msg = 'No configuration or data provided for "VAT" for category: ' + categoryConfig.name;
            context.log += msg;
            this.reportError(channel, item, msg);
            return;
        }
        ...
        if (!price) {
            const msg =
              'No configuration or data provided for "Price" for category: ' + categoryConfig.name;
            context.log += msg;
            this.reportError(channel, item, msg);
            return;
        }
        ...
        if (!depth) {
            const msg =
              'No configuration or data provided for "Package Length" for category: ' +
              categoryConfig.name;
            context.log += msg;
            this.reportError(channel, item, msg);
            return;
        ...
        if (!width) {
            const msg =
              'No configuration or data provided for "Package Width" for category: ' +
              categoryConfig.name;
            context.log += msg;
            this.reportError(channel, item, msg);
            return;
        ...
        if (!height) {
            const msg =
              'No configuration or data provided for "Package Height" for category: ' +
              categoryConfig.name;
            context.log += msg;
            this.reportError(channel, item, msg);
            return;
        ...
        if (!weight) {
            const msg =
              'No configuration or data provided for "Weight with Package" for category: ' +
              categoryConfig.name;
            context.log += msg;
            this.reportError(channel, item, msg);
            return;
        ...
        if (!name) {
            const msg =
              'No configuration or data provided for "Product Name" for category: ' +
              categoryConfig.name;
            context.log += msg;
            this.reportError(channel, item, msg);
            return;
        ...
        const categoryId = await this.getValueByMapping2(channel, categoryIdConfig, item, language, {
            systemCategoryId: ozonCategoryId
        });
        if (categoryId) {
            logger.info(
              `Overriding description_category_id from ${product.description_category_id} to ${categoryId}`
            );
        ...
        if (videoUrlsValue) {
            ...
            const videos = {
                complex_id: 100001,
                id: 21841,
                values: videoUrlsValue.map((elem: any) => {
                    return { value: elem };
                })
            };
        ...
        if (videoNamesValue) {
            ...
            const videoNames = {
                complex_id: 100001,
                id: 21837,
                values: videoNamesValue.map((elem: any) => {
                    return { value: elem };
                })
            };
        ...
        if (videoCoverValue) {
            ...
            const videoNames = {
                id: 21845,
                complex_id: 100002,
                values: videoCoverValue.map((elem: any) => {
                    return { dictionary_value_id: 0, value: elem };
                })
            };
            complex_attributes[0].attributes.push(videoNames);
            wasData = true;
        ...
        for (let i = 0; i < categoryConfig.attributes.length; i++) {
            ...
            if (
              attrConfig.id != '#productCode' &&
              attrConfig.id != '#name' &&
              attrConfig.id != '#barcode' &&
              attrConfig.id != '#price' &&
              attrConfig.id != '#oldprice' &&
              attrConfig.id != '#premprice' &&
              attrConfig.id != '#weight' &&
              attrConfig.id != '#depth' &&
              attrConfig.id != '#height' &&
              attrConfig.id != '#width' &&
              attrConfig.id != '#vat' &&
              attrConfig.id != '#videoUrls' &&
              attrConfig.id != '#videoNames' &&
              attrConfig.id != '#images360Urls' &&
              attrConfig.id != 'attr_4194' && // image attribute is filled automatically
              attrConfig.id != '#new_category' &&
              attrConfig.id != '#category' &&
              attrConfig.id != '#images' &&
              attrConfig.id != '#color_image' &&
              attrConfig.id != '#image_links_other'
            ) {
                ...
                if (attr.attributeComplexId && attr.attributeComplexId > 0) {
                    ...
                    if (!complexAttributesToProcess.includes(attr.attributeComplexId))
                        complexAttributesToProcess.push(attr.attributeComplexId);
                    continue;
                }
                try {
                    ...
                    if (value) {
                        ...
                        if (Array.isArray(value)) {
                            for (let j = 0; j < value.length; j++) {
                                ...
                                const ozonValue = await this.generateValue(
                                  channel,
                                  ozonCategoryId,
                                  ozonTypeId,
                                  ozonAttrId,
                                ...
                                if (!ozonValue) {
                                    const msg =
                                      'Value "' +
                                      elem +
                                      '" not found in dictionary for attribute "' +
                                      attr.name +
                                      '" for category: ' +
                                      categoryConfig.name +
                                      ' (' +
                        ...
                        if (!tst) product.attributes.push(data);
                            ...
                            logger.error(`Attribute ${ozonAttrId} has already been added ${JSON.stringify(tst)}`);
                    } else if (attr.required && ozonAttrId != 8229) {
                        // 8229 - old attribute - "type" that moved to type_id, required in model, but not actually
                        const msg =
                          'No value for required attribute "' +
                          attr.name +
                          '" for category: ' +
                          categoryConfig.name;
                        context.log += msg;
        ...
        if (ozonProductId && !ozonProductId.startsWith('task_id=')) {
            ...
            const newCategory = await this.getValueByMapping2(channel, newCategoryConfig, item, language, {
                systemCategoryId: ozonCategoryId
            });
            if (newCategory) {
            ...
            if (!channel.config.sendPriceUpdate) {
                ...
                if (!existingProductInfoJson)
                    existingPricesJson = await this.getProductInfo(channel, context, item, ozonProductId);
                ...
                if (priceAttr && item.values[priceAttr] != parseFloat(existingPricesJson.result.price)) {
                    if (channel.config.savePriceUpdate)
                ...
                if (
                  priceOldAttr &&
                  existingPricesJson.result.old_price &&
                  item.values[priceOldAttr] != parseFloat(existingPricesJson.result.old_price)
                ) {
                    if (channel.config.savePriceUpdate)
                ...
                if (
                  pricePremAttr &&
                  existingPricesJson.result.premium_price &&
                  item.values[pricePremAttr] != parseFloat(existingPricesJson.result.premium_price)
                ) {
                    if (channel.config.savePriceUpdate)
            ...
            if (channel.config.saveVideos) {
                ...
                if (existingDataRes.status !== 200) {
                    ...
                    const msg = 'Ozon request error: ' + existingDataRes.statusText + '   ' + text;
                    context.log += msg;
                    this.reportError(channel, item, msg);
                    logger.error(msg);
                    return;
                    ...
                    if ((videoElem1 && videoElem2) || videoCover) {
                        ...
                        const log = 'No uploaded videos found';
                        logger.info(log);
                        if (channel.config.debug) context.log += log + '\n';
                    }
                }
        ...
        if (process.env.OPENPIM_OZON_EMULATION === 'true') {
            const msg = 'Emulation mode enabled, message not sent to Ozon';
        ...
        if (res.status !== 200) {
            ...
            const msg = 'Ozon request error: ' + res.statusText + '   ' + text;
            context.log += msg;
            this.reportError(channel, item, msg);
            logger.error(msg);
            return;
            ...
            if (res2.status !== 200) {
                ...
                if (status === 'imported') {
                    context.log +=
                      'Record with identifier: ' + item.identifier + ' processed successfully.\n';
                    data.status = 4;
                    data.message =
                      'Product is under moderation ' + (errors && errors.length > 0 ? JSON.stringify(errors) : '');
                    if (errors && errors.length > 0) {
                        data.status = 3;
                        data.message = 'Errors: ' + JSON.stringify(errors);
                    }
                    ...
                    context.log +=
                      'Record with identifier: ' + item.identifier + ' processed with an error.\n';
                    data.status = 3;
                    data.message = errors && errors.length > 0 ? 'Errors:' + JSON.stringify(errors) : '';
                    item.changed('channels', true);
                } else {
                    context.log +=
                      'Record with identifier: ' + item.identifier + ' processed with status: ' + status + ' \n';
                    data.status = 4;
                    data.message = '';
                    item.changed('channels', true);
    ...
    async getProductInfo(channel: Channel, context: JobContext, item: Item, ozonProductId: string) {
        ...
        const logPr =
          'Sending request to Ozon: ' + existingProductInfoUrl + ' => ' + JSON.stringify(existingProductInfoReq);
        logger.info(logPr);
        ...
        if (existingProductInfoRes.status !== 200) {
            ...
            const msg = 'Ozon request error: ' + existingProductInfoRes.statusText + '   ' + text;
            context.log += msg;
            this.reportError(channel, item, msg);
            logger.error(msg);
            return null;
    ...
    async processItemImages(
      ...
      context: JobContext,
      product: any,
      attrs: ChannelAttribute[],
      categoryConfig: any,
      language: string
    ) {
        // image_links_other processing
        const otherImageLinksConfig = categoryConfig.attributes.find(
          (elem: any) => elem.id === '#image_links_other'
        );
        ...
        if ((channel.config.imgRelations && channel.config.imgRelations.length > 0) || images) {
            ...
            if (!typeNode) {
                throw new Error(
                  'Failed to find type by id: ' + item.typeId + ', tenant: ' + mng.getTenantId()
                );
            }
            ...
            if (type.mainImage && channel.config.imgRelations.includes(type.mainImage)) {
                const images: Item[] = await sequelize.query(
                  `SELECT a.*
                   FROM "items" a, "itemRelations" ir, "types" t
                   WHERE a."tenantId"=:tenant
                     AND ir."itemId"=:itemId
                     AND a."id"=ir."targetId"
                     AND a."typeId"=t."id"
                     AND t."file"=true
                     AND coalesce(a."storagePath", '') != ''
               AND ir."deletedAt" is null
               AND a."deletedAt" is null
               AND ir."relationId" = :relation
                ...
                if (images) {
                    for (let i = 0; i < images.length; i++) {
                        ...
                        let addition = '';
                        if (channel.config.uniqueImages)
                            addition = url.includes('?') ? '&timestamp=' + Date.now() : '?timestamp=' + Date.now();
            ...
            if (rels.length > 0) {
               ...
               AND ir."deletedAt" is null
               AND a."deletedAt" is null
               AND ir."relationId" in (:relations)
                   ORDER BY ir.values->'_itemRelationOrder', a.id`,
                  {
                      model: Item,
                      mapToModel: true,
    ...
    private async generateValue(
      ...
      value: any
        ...
        if (dictionary) {
            let dict: any[] | string | undefined = this.cache.get(
              'dict_' + ozonCategoryId + '_' + ozonAttrId + '_' + ozonTypeId
            );
            if (!dict) {
                dict = [];
                let next = false;
                ...
                do {
                    ...
                    if (ozonTypeId) {
                        ...
                        if (channel.config.debug)
                            console.log(
                              'generateValue - request to https://api-seller.ozon.ru/v1/description-category/attribute/values ' +
                              JSON.stringify(body)
                            );
                        res = await fetch('https://api-seller.ozon.ru/v1/description-category/attribute/values', {
                            ...
                            headers: {
                                ...
                                'Client-Id': channel.config.ozonClientId,
                                'Api-Key': channel.config.ozonApiKey
                            }
                        });
                    } else {
                        const body = {
                        ...
                        if (channel.config.debug)
                            console.log(
                              'generateValue - request to https://api-seller.ozon.ru/v2/category/attribute/values ' +
                              JSON.stringify(body)
                            );
                    ...
                    if (channel.config.debug)
                        console.log(
                          `generateValue - response: result length ${json.result.length}, has_next: ${json.has_next} `
                        );
                    dict = dict.concat(json.result);
                    next = json.has_next;
                    if (dict.length === 0)
                        throw new Error('No data for attribute dictionary: ' + ozonAttrId + ', for category: ' + ozonCategoryId);
                    last = dict[dict.length - 1]?.id;
                    if (idx++ > 50) {
                        throw new Error(
                          'Data dictionary for attribute: ' +
                          ozonAttrId +
                          ', typeId:' +
                          ozonTypeId +
                          ' is too big, for category: ' +
                          ozonCategoryId
                        );
                    }
                } while (next);
            ...
            if (!entry) {
                if (channel.config.debug)
                    console.log(`generateValue - entry not found 1: ${value} for attr: ${ozonAttrId}`);
                entry = (dict as any[])!.find((elem: any) => elem.id == value);
            }
            if (!entry) {
                if (channel.config.debug)
                    console.log(`generateValue - entry not found 2: ${value} for attr: ${ozonAttrId}`);
                return null;
            } else {
                if (channel.config.debug)
                    console.log(`generateValue - entry found: ${entry.id} for attr: ${ozonAttrId} `);
    ...
    public async getCategoriesOld(channel: Channel): Promise<{ list: ChannelCategory[] | null; tree: ChannelCategory | null }> {
        if (!channel.config.ozonClientId) throw new Error('Client ID not provided in channel configuration.');
        if (!channel.config.ozonApiKey) throw new Error('API key not provided in channel configuration.');
    ...
    public async getAttributesOld(channel: Channel, categoryId: string): Promise<ChannelAttribute[]> {
        ...
        if (!data) {
            ...
            logger.info(
              'Sending request to Ozon: https://api-seller.ozon.ru/v3/category/attribute => ' + JSON.stringify(query)
            );
            ...
            if (res.status !== 200) {
                const text = await res.text();
                throw new Error(
                  'Failed to query attributes with error: ' + res.statusText + ', text: ' + text
                );
            }
            const json = await res.json();
                ...
                return {
                    ...
                    dictionaryLinkPost:
                      elem.dictionary_id !== 0
                        ...
                        : null,
                    dictionaryLink:
                      elem.dictionary_id !== 0 ? 'https://api-seller.ozon.ru/v2/category/attribute/values' : null
                };
            });
    ...
    private collectTreeNew(arr: any[], treeNode: ChannelCategory, parent: any) {
        arr.forEach(elem => {
            const child = elem.type_id
              ? {
                  id: 'cat_' + parent.description_category_id + NEW_VER_DELIMETER + elem.type_id,
                  name: elem.type_name,
                  children: []
              }
              : { id: 'cat_' + elem.description_category_id, name: elem.category_name, children: [] };
            treeNode.children!.push(child);
            if (elem.children) {
                if (elem.children.length > 0) {
    ...
    public async getAttributesNew(channel: Channel, categoryId: string): Promise<ChannelAttribute[]> {
        ...
        if (!data) {
            ...
            logger.info(
              'Sending request to Ozon: https://api-seller.ozon.ru/v1/description-category/attribute => ' +
